...
export default function ToolDetailPage({ params }) {
  ...
  const authorHtml = tool.author?.link
    ? <a href={tool.author.link} target="_blank" style={{ color: 'var(--text-primary)', textDecoration: 'underline' }}>{tool.author.name}</a>
    : <span style={{ color: 'var(--text-primary)' }}>{tool.author?.name || 'VibeHub Community'}</span>;
        ...
        <div className="container" style={{ maxWidth: 800 }}>
          ...
          <div className="tool-actions-box" style={{ display: 'flex', gap: 16, justifyContent: 'center', alignItems: 'center', flexDirection: 'column' }}>
            ...
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'center' }}>
              ...
              {tool.repo && (
                <a href={tool.repo} target="_blank" className="btn btn-ghost" style={{ textDecoration: 'none' }}>
